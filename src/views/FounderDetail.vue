      ...
      <div class="ml-2">
        <div>
          ...
          <p class="mt-1 max-w-2xl text-sm leading-6 text-gray-400">
            {{ founder.role }} @
            <router-link :to="{ name: 'startupDetail', params: { startupName: founder.startupName }}" class="text-white">
              {{ founder.startupName }}
            </router-link>
          </p>
        </div>
...
// Example of getting the current startup's name or ID from a route parameter
// This will depend on your application's routing setup
const { startupName } = useRouter().currentRoute.value.params;

// Example computed property to get the list of all founders from the store
const allFounders = computed(() => store.getters.getAllFounders());

// Computed property to get cofoundersList based on the current startup
const cofoundersList = computed(() => {
  // Filter the list of all founders to find those associated with the current startup
  // Adjust this condition based on how your data is structured
  return allFounders.value.filter(founder => founder.startupName === startupName);
});
